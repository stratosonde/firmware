...
| F1 | BLOCKER | Fault handlers are brick traps; IWDG armed late | **FIXED** — fault handlers breadcrumb (magic+code) to RTC backup reg then `NVIC_SystemReset`; cause surfaces in status byte (bench gate B4) | `stm32wlxx_it.c`, `reset_cause.c` |
...
| F13 | MEDIUM | Watchdog proves liveness not progress; reset cause discarded | **FIXED** — (b) reset cause at boot rides status byte; (a) progress deadman: SendTxData marks progress (RTC_BKP_DR2), main loop resets if no cycle for 3h in FLIGHT (COMMISSIONING exempt, code 6 breadcrumb) | `lora_app.c`, `main.c`, `reset_cause.c` |
