...
| P0-3 / F14 | BLOCKER | Flash headers both in sector 0, no erase; sequence reuse; reader wedges on CRC error | **FIXED** — T4/DDR-0004 rewrite: headers in separate sectors 0/1 (v3), erase-before-write, torn-write-tolerant reader (bench gate B1 remains) | `flash_log.c`, `flash_log.h` |
| P0-4 | BLOCKER | Error_Handler bricks sonde | **FIXED** | `main.c:804` degrade-and-continue |
| P0-5/6 / F4+F5 | BLOCKER | Multi-region: only US915 joined; infinite join loop reachable in flight | **FIXED** — T1 ladder (DDR-0006): rejoin is COMMISSIONING-only; FLIGHT with no session = RF silence, profile keeps flying (GPS + flash). Bench gate B2 remains | `lora_app.c`, `multiregion_context.c` |
...
| F3 | HIGH | LSE failure = reboot loop; LSECSS trip does nothing | **FIXED (code)** — LSE failure fails over to LSI RTC clock and keeps flying. 2026-08-03: R08 found the failover was defeated by `HAL_RTC_MspInit` re-requesting LSE (backup-domain reset + dead RTC); **re-fixed @ `db4330d`** (`g_rtc_clock_source` honored). Bench gate B3 PENDING | `main.c` SystemClock_Config, `stm32wlxx_hal_msp.c` |
...
| F12 / P1-16 | HIGH | No absolute time; DDR-0003 unimplemented | **FIXED** — `SysTimeSyncFromGnss()` converts NMEA DDMMYY+HHMMSS → Unix epoch, calls `SysTimeSet` on every good fix | `lora_app.c` |
| F13 | MEDIUM | Watchdog proves liveness not progress; reset cause discarded | **FIXED (code)** — (b) reset cause at boot rides status byte; (a) progress deadman: SendTxData marks progress, main loop resets if no cycle for 3h in FLIGHT (COMMISSIONING exempt, code 6 breadcrumb). FW-4: `Deadman_Check()` also runs inside the STOP2 chunk loop + 150-chunk bound. 2026-08-03: R01/R02 found the deadman/mission-state/fault registers **collided with ST SysTime's DR0–DR2** (deadman writes poisoned the RTC time base → FLIGHT reset loop); **re-fixed @ `db4330d`** — `Core/Inc/backup_regs.h` owns the map (deadman→DR5, mission state→DR3, fault→DR4). Bench gate PENDING | `lora_app.c`, `main.c`, `reset_cause.c`, `stm32_lpm_if.c`, `Core/Inc/backup_regs.h` |
...
| F16 / N5 | HIGH | Bulk DR hardcoded DR_3; wrong SF outside US915 | **PARTIAL** — `DatarateFromSF()` resolves SF7/SF10 per active region, but 2026-08-03 review (R03/F-026) confirmed the **AU915 arm is wrong** (SF10→DR_2, SF7→DR_5; code returns DR_0/DR_3 → every AU915 bulk uplink rejected). EU868/AS923 arms verified correct. Fix: table-driven resolver + `PHY_MAX_PAYLOAD` guard. Bench gate B2 PENDING | `lora_app.c` |
...
| F22 / N11 | MEDIUM | GPS reconfig + PCAS00 flash-save on every boot | **FIXED** — gated to COMMISSIONING (DDR-0008) | `lora_app.c` LoRaWAN_Init |
...
| F26 | LOW | `flash_log.h` comment contradicts DDR-0004 | **FIXED** — header docs now describe sectors 0/1 + data from sector 2 | `flash_log.h` |
...
| FW-7 | MEDIUM | Pressure data dishonesty (failed MS5607 read transmits 1000.0 hPa sea-level default as real float-altitude data; sentinel check mis-counts a legit 1000.0 hPa as bus failure) | **FIXED** — last-known-good cache + `press_stale` flag mirroring F9 SHT31 pattern; real `ms_ok` boolean replaces sentinel; stale bit flows sensor_t → `FlashLog_Record_t.flags` (b0) → uplink status byte b5. Reset cause condensed 3-bit → 2-bit (b3–b4: POR/BOR+LP / IWDG / SW+PIN / FAULT) to free b5; DDR-0007 amended | `sys_sensors.c/h`, `reset_cause.c/h`, `payload_format.h`, `payload_encode.c`, `flash_log.c/h` |
...
| FW-13 | LOW | `vcom_Trace`/`vcom_Trace_DMA` transmit on huart1 — the GPS UART (vcom_Init already disables trace but the TX paths still lived) | **FIXED** — both TX paths no-op'd with explanatory comment (RTT is the trace path); `vcom_ReceiveInit`/`vcom_Resume` left alone (already referenced GPS-owned UART only on wake) | `usart_if.c` |
| FW-19 | LOW | `FlashLog_GetUnsentRecordsLIFO` misnamed — reads FIFO since C4 (oldest unsent first, so MarkRecordsTransmitted advances the watermark) | **FIXED** — renamed `FlashLog_GetUnsentRecordsFIFO` (impl + header + caller + comments) | `flash_log.c/h`, `lora_app.c` |
| P2-14 | LOW | Stale 300000ms comment | **FIXED** | `lora_app.h` |
| FW-20 / P2-15 | LOW | Bulk packet placeholder fields (flash_page_addr 4B, voltage_trend 10B, mode_changes 10B = 24 B of permanently-zero SF7 airtime per 222 B packet) + bogus "LIFO" docs | **FIXED** — v2 bulk packet: shrunk 222→198 B, placeholders deleted, `packet_type` 0x01→0x02, encoder signature simplified (3 placeholder params removed), docs say FIFO. Records carry their own timestamp+sequence identity. Validator updated to 198 | `payload_format.h`, `payload_encode.c`, `lora_app.c` |
| FW-11 | MEDIUM | AS923 sub-plan frequency collapse: stack compiles `CHANNEL_PLAN_GROUP_AS923_1` (offset 0) and bakes `REGION_AS923_FREQ_OFFSET` at region init — no join-time sub-plan negotiation; h3 maps all six AS923 sub-plans to `LORAMAC_REGION_AS923`. Over VN/ID (AS923-2/-3) the sonde transmits AS923-1 frequencies | **DEFERRED (per user decision — no stack change)** — candidate fixes: (a) conservative RF-silence/keep-current for AS923-2/-3/-4 cells in h3 data; (b) runtime MIB channel reconfiguration per sub-plan. Decide before any Asia-Pacific flight | `lorawan_conf.h:112`, `RegionAS923.c`, `multiregion_h3.c::H3Region_ToLoRaMacRegion` |
| FW-21 | — | Equator/prime-meridian measure-zero fix drop (exact lat=0/lon=0 fixes dropped by coordinate validation) | **DOCUMENTED, NO CHANGE** — measure-zero; a real equatorial fix crossing exactly 0.0000000 is vanishingly unlikely and a re-check would add jitter risk | `atgm336h.c` |
| FW-22 | — | Solar PB3 read with no divider (raw ADC) | **HARDWARE CHECK, NOT CODE** — verify panel Voc ≤ VDDA+0.3 V abs-max on the production board before launch (two-wafer ~1.1 V panel expected; already in bench checklist spirit) | `adc_if.c` solar path |
| P2-15 (remnants) | LOW | Geofence dead code, blocking delays | **OPEN** | various |
