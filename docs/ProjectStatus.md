...
Last updated: 2026-08-15 (regenerated from current evidence, master @ `4014d33`) — **the 2026-08-13/14 stability + LT reviews have been executed: all P0/P1/P2 code findings are fixed (#269–#278), wave-2 findings are tracked (#279–#289), the requirements/docs layer carries dated statuses, host baseline is 13 suites green + sanitize, and CI is green (host-tests, firmware, firmware-flight). ARM text 198,764 B.** Prior state: 2026-08-07 refactor batch + doc sweep (#75/#73/#74/#77, #40).
...
| **#282** (code, high) | H-03: LoRaWAN NVM two-slot store is not power-cut atomic — needs persistence design + own host tests |
...
| **#279, #280, #281, #283, #289** (code, normal) | LT-09 solar flag (batches with #266) · M-02 config threshold consumption · H-01 durable pre-flight state (design) · H-05 sticky CtxRestoreDone · M-01 deferred ring reconstruction |
...
3. **#282 needs its own host tests first** — the NVM slot store has zero host coverage today; write the torn-slot/generation tests before touching the store (they are the red-first gate).
