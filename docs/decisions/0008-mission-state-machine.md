...
- **FLIGHT.** Everything after release. LEDs permanently off (wasted power). Join unreachable. GPS reconfigure unreachable — the receiver's persisted config (saved during commissioning) is trusted.
  - Optional sub-phase **ASCENT → FLOAT**, one-way: ascent samples/transmits fast (5–10 s cadence) to capture the climb, then settles to float cadence. Transition by **timer by default** (most robust — cannot be fooled by any sensor), optionally a simple altitude check (above threshold and no longer climbing for a sustained period, with hysteresis). **Implementation note (D8/#59, FR-17/#98):** the transition is pressure-trend based, not a timer — FLOAT when |ΔP| stays below **2% of ambient pressure** for 3 consecutive work cycles. The threshold is deliberately *relative*: a fixed 2 hPa is ~17% of float ambient (false FLOAT while climbing) yet only ~18 m at sea level (false FLOAT on the launch pad).
