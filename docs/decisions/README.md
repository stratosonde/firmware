...
**Date:** 2026-08-09 (last merged: 2026-08-13 intent interview; divergence list annotated 2026-08-15 against master @ `dc8026a`)  
...
1. ~~`mission_state.c` implements legacy 0008 (ASCENT/FLOAT, manual enter-flight) — DDR-0002/0018 require cadence dynamics + automatic launch detection.~~ **Reframed 2026-08-13:** the explicit ASCENT/FLOAT one-way latch is now the *required* model (DDR-0002 `INV-LIFE-011`), so `mission_state.c` is conformant on that axis. What remains is confirming automatic launch detection works without the operator action (DDR-0018 `P-COMM-013`). **2026-08-15:** host evidence now exists (`mission_logic` suite covers autonomous launch detection; C-01/#270 gates the flight door on durable provisioning). Target confirmation open.
2. `transmit_plan.c` GPS veto treats stale temperature as COLD (cites legacy 0007) — DDR-0016 requires last-known-good; the cold-lockout itself is removed entirely by DDR-0021. **Resolved (verified 2026-08-15):** no temperature term reaches the GPS-enable path anymore; a stale temperature falls back to the raw battery reading (R10/#37) and the cold lockout is gone per DDR-0021.
3. `power_model.c` disables GPS by operating mode — DDR-0016 requires cadence-first + per-operation droop admission. **Sharpened 2026-08-13:** DDR-0016 §11 / DDR-0001 §17 forbid energy policy *deliberately* planning a GNSS-less wake; per-wake admission must be FULL or SLEEP. **2026-08-15:** still open — tracked in #288 (H-12) together with the durable-trend gap.
4. Region hold is indefinite in code — DDR-0015 requires the staleness budget and Band-3 silence. **Partially closed 2026-08-13:** the budget is now bound (24 h) and `GPS_LOSS_SILENCE_S` was retimed to match, so a silence path exists. Still open: Band-0 `home_region` fallback, and confirming held-region expiry follows the same single budget. **2026-08-15:** Band-0 → #287 (H-11); same-wake fresh-fix recovery → #285 (H-09).
5. No open-ocean ring search (`multiregion_h3.c` is direct lookup only) — DDR-0006. **Reconfirmed 2026-08-13** as a conformance defect, not a doc gap: retaining the previous region in no-region geography instead of running the bounded ring search violates DDR-0006/0015. **2026-08-15:** appears resolved in code — `multiregion_h3.c` now runs a bounded nearest-neighbour search (≤ 3 rings) on the "not in any region" path. The 2026-08-13 reconfirmation looks stale; one behavioural boundary test closes this.
6. Archive delivery follows legacy 0011 semantics — DDR-0005 requires send-once watermark + backend gap-repair hooks. **Extended 2026-08-13:** also needs explicit-record-request preemption and ephemeral (non-persisted) request handling (DDR-0005 `INV-TX-010`). **2026-08-15:** the send-once watermark + one-pass recovery landed (R3-04/#218); explicit record requests remain unimplemented (no lookup API — FW-CONF-042..045); a fresh-outage backlog drains oldest-first → #286 (H-10).
7. **New 2026-08-13:** no reset-count/failure-history escalation may exist anywhere (DDR-0009 `OD-FAIL-006` resolved, DDR-0020). Any repeated-reset adaptation should be removed or disabled for first flight unless separately re-decided. **2026-08-15:** implemented as degrade-not-escalate (FR-23/#104: boot-attempt counter resets on a completed work cycle; the degrade escape replaces the 6th reset). Formal audit open (FW-CONF-041).
8. **New 2026-08-13:** archive metadata recovery must not be an unconditional boot prerequisite, and an isolated torn record must not truncate the archive (DDR-0011 `INV-STORE-011`, `BR-STORE-001/002`; DDR-0012 `INV-BOOT-010`). **2026-08-15:** torn-record skip implemented (F-006/#51); metadata recovery is still a synchronous boot prerequisite on double-header loss → #289 (M-01).
...
4. **Battery/load profiling campaign** — hardware task unblocking DDR-0016 OD-PWR-001. **2026-08-15:** also feeds #248 (the −40/−50 °C compensation-table bench data); see the readiness checklist §9/§10.
...
6. **Proof work from the 2026-08-13 merge** — the newly confirmed requirements need executable evidence before the code can be called conformant. Queued in `../requirements/firmware-conformance-worklist.md` (FW-CONF-023 onward) and `../requirements/requirements-traceability-matrix.md`. **2026-08-15:** both documents now carry dated per-item statuses; the LT/C-01 sweep closed the 2026-08-13/14 review's P0-P2 code items (#269-#278).
7. **DDR-0027 bindings** — protocol-version identification encoding (`OD-PROTO-001`) and the backend codec retirement horizon (`OD-PROTO-002`) must be decided with DDR-0019 payload bindings. **2026-08-15:** the next wire-format bump batches in #266 (and #279's solar staleness bit).
