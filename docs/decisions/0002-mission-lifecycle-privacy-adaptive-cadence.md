...
- **FLOAT detection is windowed-range (5% over 900 s) with NO altitude guard** - float altitude is payload/balloon-dependent (5-25 km), so no fixed ceiling pressure can be correct. Pad-side protection is structural: detection only runs in ASCENT, and ASCENT requires arming or launch.
