 ...
 * Brace placement is clang-format-neutral: both 'sig) {' and 'sig)\n{' are
 * accepted, because the box's clang-format 22.1.8 sometimes normalizes. */
static bool in_function(const char *src, const char *sig, const char *needle) {
  const char *start = strstr(src, sig);
  if (!start) {
    /* Try the clang-format alternate brace placement: ') {' vs ')\n{'. */
    size_t n = strlen(sig);
    if (n >= 2 && sig[n - 1] == '{') {
      char alt[256];
      if (sig[n - 2] == '\n') {
        snprintf(alt, sizeof(alt), "%.*s {", (int)(n - 2), sig);
        start = strstr(src, alt);
      }
    }
  }
  if (!start) {
    printf("FATAL: signature not found: %s\n", sig);
    exit(2);
  }
...
static void test_f6_boot_attempts_placement(const char *app) {
  ...
  bool marked_first =
      in_function(app, "static void SendTxData(void) {", "Deadman_MarkProgress();") ||
...
static void test_f8_battery_sample_coherence(const char *app, const char *sens) {
  ...
  bool plan_uses_foreign_flag =
      in_function(app, "static void SendTxData(void) {", "sensor_data.batt_stale") ||
...
static void test_r9_poweron_transaction(void) {
  ...
  for (int i = g_host_gpio_log_n - 1; i >= 0; i--) {
    if (pwr_final < 0 && g_host_gpio_log[i].pin == g.pwr_pin)
      pwr_final = g_host_gpio_log[i].state;
    if (en_final < 0 && g_host_gpio_log[i].pin == g.en_pin)
      en_final = g_host_gpio_log[i].state;
  }
