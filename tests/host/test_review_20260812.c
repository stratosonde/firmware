...
static void test_sa_gps_loss_retry_budget(const char *app) {
  ...
  if (!found) {
    printf("FATAL: S-A anchor missing\n");
    exit(2);
  }
...
/* ========================================================================== */
static void test_sd_commission_chunk_bound(const char *lpm) {
  printf("-- SP-06 (#256): commissioning uses 1 s WUT chunk, flight 20 s\n");
  CHECK_REGRESSION(strstr(lpm, "POLLING_CHUNK_COUNTS") != NULL, "SP-06-poll-count");
  CHECK_REGRESSION(strstr(lpm, "IWDG_WAKEUP_COUNTS") != NULL, "SP-06-flight-count");
  CHECK_REGRESSION(strstr(lpm, "MissionState_IsCommissioning()") != NULL,
                   "SP-06-select");
}
...
int main(void) {
  ...
  test_sd_commission_chunk_bound(lpm);
  printf("\n");
