...
/* M-01 (#289): flight gate — SONDE_LOG expands to no-op here. Without it the
 * deferred-reconstruction log lines (like the pre-existing RECONSTRUCTED line)
 * would end up unresolved SEGGER_RTT references at link. */
#define SONDE_FLIGHT_BUILD
...
static void test_r2_03_sequence_identity_crosscheck(void) {
  ...
  CHECK_EQ_I(W25Q_Read(&hw, FLASH_LOG_DATA_START + 2U * FLASH_LOG_RECORD_SIZE,
                       (uint8_t *)&planted, sizeof(planted)),
             W25Q_OK);
  planted.sequence = 99;
...
/* ========================================================================== */
/* M-01 (#289) — deferred ring reconstruction on double header loss            */
/* ========================================================================== */
/* UNSVERIFIED-until-here review copy: whole-ring recovery is NOT a boot
 * prerequisite. On double header loss Init must probe only a bounded window
 * (FLASH_LOG_RECON_PROBE_SLOTS) and boot in a deferred state; the deferred
 * scan runs in bounded chunks (FLASH_LOG_RECON_STEP_SLOTS) via
 * FlashLog_ReconstructStep() on energy-eligible wakes; completion reconciles
 * the frontier (record_count/next/pending/drain) monotonically upward in its
 * clamp sweep and persists the header. */
static void test_m01_deferred_reconstruction(void) {
  printf("-- M-01 (#289): double header loss -> deferred (bounded) reconstruction\n");
  ...
  /* 400 records: more than the bounded probe window (256) but far below a
   * full ring fill, and deep enough that the stale tail (320..399) survives
   * the write-path erase-ahead sector (256..319) - so the deferred scan must
   * RECONCILE the provisional frontier upward (see below). */
  for (uint32_t i = 0; i < 400; i++) {
    sensor_t s = make_sensors((uint16_t)i);
    CHECK_EQ_I(FlashLog_WriteRecord(&hlog, &s, 5000U + i, 0, 0, 0), FLASH_LOG_OK);
  }
  ...
  /* Init must only probe the bounded window and boot in a deferred state. */
  FlashLog_HandleTypeDef rec;
  CHECK_EQ_I(FlashLog_Init(&rec, &hw), FLASH_LOG_OK);
  CHECK(rec.reconstruct_pending);
  CHECK_REGRESSION(rec.record_count == FLASH_LOG_RECON_PROBE_SLOTS, "M-01-provisional");

  /* Science logging proceeds immediately. */
  sensor_t s = make_sensors((uint16_t)7);
  CHECK_EQ_I(FlashLog_WriteRecord(&rec, &s, 6000U, 0, 0, 0), FLASH_LOG_OK);
  CHECK(FlashLog_HasUnsentData(&rec));

  /* Bounded chunk API converges. */
  uint32_t guard = 0;
  while (rec.reconstruct_pending && guard++ < 1000U) {
    CHECK_EQ_I(FlashLog_ReconstructStep(&rec), FLASH_LOG_OK);
  }
  CHECK(!rec.reconstruct_pending);
  /* Completion lifted the frontier monotonically to max-seen (399: the
   * stale tail 320..399 survived the erase-ahead sector and the deferred
   * scan found it; identity lifts from the provisional 256/257 -> 400). */
  CHECK_REGRESSION(rec.record_count == 400U, "M-01-reconciled");

  /* Completion persisted the header: a fresh Init does NOT re-defer. */
  FlashLog_HandleTypeDef rec2;
  CHECK_EQ_I(FlashLog_Init(&rec2, &hw), FLASH_LOG_OK);
  CHECK(!rec2.reconstruct_pending);
  CHECK_REGRESSION(rec2.record_count == 400U, "M-01-persisted");
...
int main(void) {
  ...
  test_m01_deferred_reconstruction();
