/* ARCHIVE — regression record for the 2026-08-13 SP review. Module contracts
...
static void test_sp07_fatal_escape_wired(const char *mainc) {
  ...
  CHECK_REGRESSION(count_occurrences(mainc,
                                     "Error_Handler_Fatal(FAULT_CODE_FLASH_INIT)") >= 2,
                   "SP-07-sites");
...
static void test_sp14_nan_guards(const char *pend) {
   ...
   * forward prototype at the file top - the notes' scan pit). The return
   * type is uint8_t (the field is unsigned 0-255; the int8_t declaration
   * was a type lie fixed in the wire-hygiene pass). */
  CHECK_REGRESSION(in_function(pend, "static uint8_t ConvertTemperatureToCompact(float temperature_c) {",
                               "isnan"),
                   "SP-14-temp-guard");
