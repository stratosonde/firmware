/* ARCHIVE — regression record for the cross-review findings (STAB-01 et al.).
...
static void test_lse_failover_deferred_and_complete(void) {
  ...
  if (fo == NULL)
    fo = strstr(src, "MX_RTC_Init(); /* backup domain was reset");
  CHECK(fo != NULL);
  if (fo) {
    char window[700];
...
static void test_f4_liveness_and_failover_transactional(void) {
  ...
  if (lv == NULL)
    lv = strstr(src, "static void RTC_LivenessCheck(void)\r\n{");
  if (lv == NULL)
    lv = strstr(src, "static void RTC_LivenessCheck(void) {");
  CHECK(lv != NULL);
  if (lv) {
  ...
  if (fo) {
    ...
    CHECK_REGRESSION(strstr(fo - 60 > src ? fo - 60 : src, "ck_status") != NULL ||
                         strstr(fo - 60 > src ? fo - 60 : src, "!= HAL_OK") != NULL,
                     "F4-hal-check");
  }
  free(src);
}
